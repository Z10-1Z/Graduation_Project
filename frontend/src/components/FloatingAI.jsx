...
function FloatingAIInner({ role = 'patient' }) {
  ...
  const ensureConversation = useCallback(async () => {
    ...
    const conversation = await createChatConversation({ surface: 'floating_widget', role }, { token });
    setConversationId(conversation.id);
    return conversation.id;
  }, [conversationId, role, token]);

  const sendMessage = async (text) => {
    const cleanText = text.trim();
    if (!cleanText || streaming) return;

    const userMsg = { id: createMessageId(), from: 'user', text: cleanText, time: getCurrentTime() };
    ...
    if (!token) {
      setMessages((prev) => [
        ...prev,
        { id: assistantId, from: 'ai', text: t('ai.floating.loginRequired'), time: getCurrentTime() },
      ]);
      setLoading(false);
      setStreaming(false);
      return;
    ...
    try {
      ...
      await sendChatMessageStream({
        ...
        onDelta: (delta) => {
          if (!assistantStarted) {
            ...
            setMessages((prev) => [
              ...prev,
              { id: assistantId, from: 'ai', text: '', time: getCurrentTime() },
            ]);
          }
      ...
      if (!assistantStarted) {
        setMessages((prev) => [
          ...prev,
          { id: assistantId, from: 'ai', text: t('ai.floating.emptyResponse'), time: getCurrentTime() },
        ]);
      }
    } catch (err) {
      setMessages((prev) => [
        ...prev,
        { id: assistantId, from: 'ai', text: getApiErrorMessage(err, t('ai.floating.sendError')), time: getCurrentTime() },
      ]);
    } finally {
      setLoading(false);
      setStreaming(false);
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
              {quickActions.map((action) => (
                <button
                  ...
                  disabled={streaming}
                  className="flex-shrink-0 text-xs bg-blue-50 text-blue-600 font-semibold px-3 py-1.5 rounded-xl hover:bg-blue-100 transition-colors whitespace-nowrap disabled:opacity-50"
                >
                  {action}
                </button>
              ))}
